  ...
  const handleDownloadImages = async () => {
    ...
    try {
      ...
      if (successfulCompressions.length === 1) {
        ...
        const blob = new Blob([new Uint8Array(image.data)], { type: `image/${image.format}` });
      ...
      successfulCompressions.forEach(image => {
        ...
        const baseName = image.originalName.replace(/\.[^/.]+$/, "");
        const fileName = `${baseName}.${image.format}`;
        zip.file(fileName, new Uint8Array(image.data));
      });
    ...
    >
      <motion.div variants={fadeInUp} className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold">
          <span className="gradient-text">ImageZero</span>
        </h1>
        <p className="text-muted-foreground">
          Compress multiple images while maintaining quality with ImageZero's intelligent algorithms
        </p>
      </motion.div>
            ...
            <CardContent className="p-6">
              <div className="flex flex-col gap-6">
                ...
                <Button
                  ...
                  className="mt-2 bg-linear-to-b h-10 text-md from-[#353f5b] to-[#232a40] hover:shadow-[0_5px_10px_rgba(109,120,161,0.6)] transition-all duration-200"
                >
                  {isLoading ? (
                    <>
                ...
                <div className="mt-8">
                  ...
                  <div className="max-h-[400px] overflow-y-auto pr-1 space-y-3">
                    {selectedImages.map((image) => (
                      ...
                      >
                        <div className="h-12 w-12 bg-muted rounded-lg overflow-hidden shrink-0 mr-3">
                          <img
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-8 w-8 p-0 rounded-full text-muted-foreground hover:text-destructive shrink-0 ml-2"
          ...
          <Card className="overflow-hidden border-border/50 shadow-lg hover:shadow-xl transition-shadow h-full">
            <CardContent className="p-6">
              <div className="flex flex-col gap-6 h-full">
                  ...
                  <div className="bg-background/50 p-4 rounded-xl border border-border/40 backdrop-blur-sm">
                    ...
                    <div className="w-full bg-secondary rounded-full h-2 overflow-hidden">
                      <motion.div 
                        ...
                        className="bg-linear-to-r from-primary to-blue-500 h-full"
                      ></motion.div>
                    </div>
                ...
                {compressedImages.length > 0 ? (
                  <div className="mt-2 max-h-[500px] overflow-y-auto pr-1 space-y-3 grow">
                    {compressedImages.map((image) => (
                      ...
                      >
                        <div className="flex-1 flex items-center gap-3">
                          <div className="h-10 w-10 flex items-center justify-center shrink-0 bg-muted/30 rounded-lg">
